<x-app-layout>
    <div class="min-h-screen bg-gray-100  dark:bg-gray-900 py-6 flex flex-col justify-center sm:py-12">
        <div class="relative py-3 sm:max-w-xl sm:mx-auto">
            <div class="absolute inset-0 bg-gradient-to-r from-red-400 to-orange-600 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl"></div>
            <div class="relative px-4 py-10 bg-white dark:bg-gray-800 shadow-lg sm:rounded-3xl sm:p-20">
                <h1 class="text-3xl font-bold text-gray-900  dark:text-gray-100 mb-6">Editar Ingreso</h1>

                <form id="edit-ingreso-form" action="{{ route('ingresos.update', $ingreso->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="descripcion" class="block text-sm font-medium text-gray-700 dark:text-gray-100">Descripción</label>
                        <input type="text" id="descripcion" name="descripcion" value="{{ $ingreso->descripcion }}" required
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" id="monto" name="monto" value="{{ $ingreso->monto }}" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="fecha" class="block text-sm font-medium text-gray-700 dark:text-gray-100">Fecha</label>
                        <input type="date" id="fecha" name="fecha" value="{{ $ingreso->fecha }}" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
    ...
    <script>
        ...
        Swal.fire({
            ...
            html: 'Estás a punto de actualizar este ingreso.<br><small style="color: #6b7280">Los cambios serán permanentes.</small>',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    didOpen: () => {
                        Swal.showLoading();
                        setTimeout(() => {
                            document.getElementById('edit-ingreso-form').submit();
                        }, 500);
                    }
                });
            }
